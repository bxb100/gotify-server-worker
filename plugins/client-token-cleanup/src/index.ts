...
export class ClientTokenCleanupPlugin extends GotifyPlugin {
  ...
  public getDisplay(state: PluginState, config: unknown): string {
    ...
    if (!state.enabled) {
      lines.push(
        '',
        'Enable this plugin from the plugin list to start scheduled cleanup.'
      )
    }
...
const pluginDefinition: PluginDefinition = {
  manifest,
  defaultConfig,
  plugin: ClientTokenCleanupPlugin
}
...
async function runCleanup(
  ...
  const cutoff = new Date(
    Date.now() - config.lastLoginOlderThanDays * 24 * 60 * 60 * 1000
  ).toISOString()

  try {
    const candidates = await host.getClientsLastUsedBefore(cutoff)
    const matched = candidates.filter((client) =>
      config.clientNamePattern.test(client.name)
    )
    ...
    console.log(
      `client token cleanup finished at ${ranAt}, deleted=${deletedCount}`
    )
  } catch (error) {
    const message =
...
function parseConfig(config: unknown): ParsedConfig {
  const object = requireConfigObject<CleanupConfig>(config, [
    'clientNameRegex',
  ...
  try {
    ...
    throw new Error(
      error instanceof Error
        ? `invalid clientNameRegex: ${error.message}`
        : 'invalid clientNameRegex',
      {
        cause: error
      }
    )
