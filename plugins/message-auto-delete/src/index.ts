...
async function runCleanup(
  host: MessageAutoDeletePlugin['host'],
  ...
  try {
    ...
    console.log(
      `message retention cleanup finished at ${ranAt}, deleted=${deletedCount}`
    )
  } catch (error) {
    const message =
...
function parseConfig(config: unknown): { retentionDays: number } {
  ...
  if (
    typeof object.retentionDays !== 'number' ||
    !Number.isInteger(object.retentionDays) ||
    object.retentionDays < 1
  ) {
    throw new Error(
      'retentionDays must be an integer greater than or equal to 1'
    )
  }
