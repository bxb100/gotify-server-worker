...
export function buildNextUrl(currentUrl: string, limit: number, since: number): string {
...
export function boolFromEnv(value: string | undefined, fallback = false): boolean {
	if (value === undefined) {
		return fallback;
	}
...
export function getBasicCredentials(request: Request): { name: string; pass: string } | null {
	const authorization = request.headers.get("Authorization");
	if (!authorization) {
		return null;
	}
	if (authorization.length < 6 || authorization.slice(0, 6).toLowerCase() !== "basic ") {
		return null;
	}
...
export function toApplicationExternal(app: ApplicationRow): ApplicationExternal {
	return {
		id: app.id,
		token: app.token,
		name: app.name,
...
export function corsHeaders(originHeader: string | null, allowOrigin: string | undefined): Headers {
	const headers = new Headers();
	const allowValue = allowOrigin?.trim() || originHeader || "*";
	headers.set("Access-Control-Allow-Origin", allowValue);
	headers.set("Access-Control-Allow-Headers", "Authorization, Content-Type, X-Gotify-Key");
