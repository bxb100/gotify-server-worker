...
app.use("*", async (c, next) => {
	...
	const headers = corsHeaders(c.req.header("Origin") ?? null, c.env.CORS_ALLOW_ORIGIN);
...
app.post("/user", optionalAuth, async (c) => {
	...
	try {
		const created = await createUser(c.env.DB, {
			name,
			pass: await hash(pass, passwordRounds(c.env.GOTIFY_PASSWORD_ROUNDS)),
			admin,
		});
		...
		const updated = await updateUser(c.env.DB, {
			...
			pass:
				typeof body.pass === "string" && body.pass !== ""
					? await hash(body.pass, passwordRounds(c.env.GOTIFY_PASSWORD_ROUNDS))
					: existing.pass,
		});
...
app.post("/current/user/password", requireClient, async (c) => {
	...
	await updateUser(c.env.DB, {
		...
		pass: await bcrypt.hash(requireNonEmptyString(body.pass, "pass"), passwordRounds(c.env.GOTIFY_PASSWORD_ROUNDS)),
	});
	return new Response(null, { status: 200 });
});
...
app.put("/application/:id", requireClient, async (c) => {
	...
	try {
		const updated = await updateApplication(c.env.DB, {
			...
			description: maybeString(body.description),
			defaultPriority: maybeNumber(body.defaultPriority, 0),
			sortKey: typeof body.sortKey === "string" && body.sortKey.trim() !== "" ? body.sortKey : appRow.sort_key,
			image: appRow.image,
		});
...
app.delete("/application/:id/image", requireClient, async (c) => {
	...
	if (!appRow.image) {
		throw new ApiError(400, `app with id ${id} does not have a customized image`);
	}
...
app.post("/message", requireApplication, async (c) => {
	...
	if (!application) {
		throw new ApiError(401, "you need to provide a valid access token or user credentials to access this api");
	}
...
function buildPagedMessages(
	...
	return {
		paging: {
			...
			next: hasNext && last ? buildNextUrl(requestUrl, limit, last.id) : undefined,
		},
		messages: visible.map(toMessageExternal),
	};
}

async function publishToUserStream(env: EnvBindings, userId: number, payload: unknown): Promise<void> {
