...
import { ApiError, getBasicCredentials, getTokenFromRequest, isOlderThan } from "./utils";
...
async function getBasicUser(env: EnvBindings, request: Request): Promise<UserRow | null> {
	...
	return (await bcrypt.compare(credentials.pass, user.pass)) ? user : null;
}

function setAuth(c: { set: (key: "auth", value: AuthState) => void }, auth: AuthState): void {
	c.set("auth", auth);
}
...
export const optionalAuth = createMiddleware<AppContext>(async (c, next) => {
	...
	if (basicUser) {
		setAuth(c, { kind: "basic", userId: basicUser.id, token: "", user: basicUser });
		await next();
		return;
	}
...
export const requireApplication = createMiddleware<AppContext>(async (c, next) => {
	const basicUser = await getBasicUser(c.env, c.req.raw);
	if (basicUser) {
		throw new ApiError(403, "you are not allowed to access this api");
	}
