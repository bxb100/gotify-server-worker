...
export class ClientStore extends BaseStore<IClient> {
  ...
  protected requestItems = (): Promise<IClient[]> => requestJson<IClient[]>(`${config.get("url")}client`);

  protected requestDelete(id: number): Promise<void> {
    return requestVoid(`${config.get("url")}client/${id}`, { method: "DELETE" }).then(() =>
  ...
  public update = async (id: number, name: string): Promise<void> => {
    await requestVoid(`${config.get("url")}client/${id}`, { method: "PUT", body: { name } });
    await this.refresh();
