...
const Clients = observer(() => {
	...
	return (
		<DefaultPage
			title="Clients"
			rightControl={
				<Button id="create-client" variant="contained" color="primary" onClick={() => setCreateDialog(true)}>
					Create Client
				</Button>
			}
		>
			...
			{createDialog && <AddClientDialog fClose={() => setCreateDialog(false)} fOnSubmit={clientStore.create} />}
			{toUpdateClient != null && (
				<UpdateClientDialog
...
const Row = ({ name, value, lastUsed, fEdit, fDelete }: IRowProps) => (
	<TableRow>
		<TableCell>{name}</TableCell>
		<TableCell>
			<CopyableSecret value={value} style={{ display: "flex", alignItems: "center", width: 250 }} />
		</TableCell>
		<TableCell>
