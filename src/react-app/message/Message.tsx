...
const Message = ({
	...
	return (
		<div className={`${classes.wrapperPadding} message`}>
			...
			>
				{smallHeader ? (
					<HeaderSmall fDelete={fDelete} title={title} appName={appName} image={image} date={date} />
				) : (
					<HeaderWide fDelete={fDelete} title={title} appName={appName} image={image} date={date} />
				)}
				...
				<div className={classes.messageContentWrapper}>
					<Typography
						component="div"
						ref={onContentRef}
						className={`${classes.content} content ${isOverflowing && expanded ? "expanded" : ""}`}
					>
						{renderContent()}
					</Typography>
				</div>
...
const HeaderWide = ({
	...
	return (
		<div className={classes.header}>
			...
			<IconButton onClick={fDelete} style={{ padding: 14 }} className={`${classes.trash} delete`} size="large">
				<Delete />
			</IconButton>
		</div>
	);
		...
		<div className={classes.header}>
			...
			<div style={{ display: "flex", alignItems: "end", flexDirection: "column" }}>
				...
				<div style={{ width: 30, height: 30 }}>
					{image !== null ? (
						<img src={config.get("url") + image} alt={`${appName} logo`} className={classes.image} />
					) : null}
				</div>
			</div>
		</div>
