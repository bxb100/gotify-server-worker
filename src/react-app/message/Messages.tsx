...
const Messages = observer(() => {
	...
	const deleteMessage = (message: IMessage) => {
		const key = enqueueSnackbar({
			...
			variant: "info",
			action: () => (
				<Button color="inherit" size="small" onClick={() => messagesStore.cancelPendingDelete(message)}>
					Undo
				</Button>
			),
		...
		>
			...
			{pushMessageOpen && app && (
				<PushMessageDialog
					...
					fOnSubmit={(message, title, priority) => messagesStore.sendMessage(app.id, message, title, priority)}
				/>
			)}
		</DefaultPage>
	);
