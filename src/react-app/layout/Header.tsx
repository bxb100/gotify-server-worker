...
const Header = ({ name, loggedIn, admin, toggleTheme, logout, style, setNavOpen, showSettings, themeMode }: IProps) => {
  const { classes } = useStyles();
  const themeLabel = `Toggle theme (current: ${themeMode})`;
  const themeIcon = themeIcons[themeMode];
  return (
    <AppBar sx={{ position: { xs: "sticky", sm: "fixed" } }} style={style} className={classes.appBar}>
      <Toolbar className={classes.toolbar}>
        ...
        {loggedIn && (
          <Buttons admin={admin} name={name} logout={logout} setNavOpen={setNavOpen} showSettings={showSettings} />
        )}
        <div>
...
const Buttons = ({
  ...
  return (
    <div className={classes.menuButtons}>
      ...
      <ResponsiveButton icon={<AccountCircle />} label={name} onClick={showSettings} id="changepw" color="inherit" />
      <ResponsiveButton icon={<ExitToApp />} label="Logout" onClick={logout} id="logout" color="inherit" />
    </div>
  );
};
