...
export class PluginStore extends BaseStore<IPlugin> {
	...
	public requestConfig = (id: number): Promise<string> => requestText(`${config.get("url")}plugin/${id}/config`);

	public requestDisplay = (id: number): Promise<string> => requestText(`${config.get("url")}plugin/${id}/display`);

	protected requestItems = (): Promise<IPlugin[]> => requestJson<IPlugin[]>(`${config.get("url")}plugin`);
